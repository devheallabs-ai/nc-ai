...
- `nc` binary v1.1.0 or higher ([build from nc-lang](../nc-lang/INSTALL.md) or download a release)
- Git
- PowerShell (Windows) or Bash (macOS/Linux) for the test suite
- Optional: Node.js 18+ (only needed for tooling scripts)
...
```bash
# Build nc first if needed
cd ../nc-lang/engine && make && cd -

export PATH="$PWD/../nc-lang/engine/build:$PATH"
