...
NC_BIN=./nc-lang/engine/build/nc bash nc-ai/tests/run_tests.sh

# Plain log-friendly output
NO_COLOR=1 bash nc-ai/tests/run_tests.sh
...
cd /c/path/to/nc-main/nc-lang/engine
make
# Produces: build/nc.exe
```
...
- Shell test runners default to the repo-local `nc-lang/engine/build/nc(.exe)` binary and also accept `NC_BIN` overrides.
...
```bash
# Add corpus files to nc-ai/training_data/nc_corpus/
# Then retrain:
python3 nc-ai/nova/training/enhance_nova.py

# Or rebuild corpus from Documents:
bash nc-ai/training_data/build_corpus.sh
python3 nc-ai/nova/training/enhance_nova.py
```
...
**Tests fail?**
- Run `bash nc-ai/launch.sh build` to rebuild
- Check `nc-lang/engine/build/nc` exists
