...
**Training data:** 38 corpus files in `nc-ai/training_data/nc_corpus/`

**Model artifacts:** `nc-ai/nova/model/`

| Artifact | Description |
|----------|-------------|
| `metadata.json` | Model version, stats, config |
| `enhanced_triplets.json` | 17K+ knowledge triplets |
| `lm_5gram.pkl` | 5-gram language model (917K 5-grams) |
| `node_embeddings.npy` | 14,805 node embeddings (64-dim) |
| `node_to_idx.json` | Node name → embedding index |
| `confidence_calibration.json` | Logistic calibration params |
| `ner_entities.json` | 5,401 named entities |
| `path_attention.json` | Learned attention weights |
| `concept_hierarchy.json` | 316 concept clusters |

**Retrain the model:**
```bash
python3 nc-ai/nova/training/enhance_nova.py
```

**Important:** Python is only used for offline training. At runtime, the C engine loads the model artifacts directly. Users never need Python — everything runs through NC.
...
No. Python is only used for one-time model training (offline). At runtime, everything is NC + C.
...
Yes. See `nc-lang/Dockerfile` and `nc-ai/Dockerfile` for container builds.
